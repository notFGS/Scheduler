...
const calculateGridPosition = (course, minHour) => {
  const startHour = convertToFractionalHour(course.fromTime);
  const endHour = convertToFractionalHour(course.toTime);
  ...
  const dayColumn = days.indexOf(course.day) + 1; // Day columns should start from 1 (Sunday)
      ...
      <div className="course-blocks">
        {groupedCourses.map((group, groupIndex) => {
          return group.map((course, courseIndex) => {
            const { startRow, endRow, dayColumn } = calculateGridPosition(course, 9); // 9 is the minimal hour (09:00)
            return (
              <div
                key={course.id}
